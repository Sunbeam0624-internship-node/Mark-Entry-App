...
router.post("/register", (request, response) => {
  ...
  const { empNo, staffName, email, password, courseName} = request.body;
  console.log("***: ", empNo, staffName, email, password,courseName);
  ...
  const statement = `INSERT INTO staff 
    (employee_number, staff_name, email, password,course_name)
    VALUES (?, ?, ?, ? ,?)`;
  ...
  db.execute(
    statement,
    [empNo, staffName, email, encryptedPassword , courseName],
    (error, result) => {
...
router.post("/login", (request, response) => {
  ...
  const statement = `SELECT staff_id, employee_number, staff_name, email , role , course_name 
      FROM staff
  ...
  db.execute(statement, [email, encryptedPassword], (error, users) => {
    if (error) {
      ...
      if (users.length === 0) {
        ...
        console.log("Check Staff payload ", payload);
